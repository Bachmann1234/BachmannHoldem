...
- [x] The session `Model` gains a retained, append-only log of the session's graded coach decisions —
      ...
      say "in hand #7…".
- [x] Only **graded** rulings are retained — `{ kind: 'none' }` and `{ kind: 'error' }` carry no spot
      and are not logged.
- [x] The reducer appends to the log at the **same point** it currently sets `model.coach` for a graded
...
- [x] The log is reset when a **new session** starts (`setup` / new game), not between hands.
- [x] The retained log is available on the model in the `session-over` / `game-over` phases for
      [[0109-coach-session-synthesis]] to read.
- [x] Co-located reducer tests cover: a multi-hand session accumulates one entry per graded hero
