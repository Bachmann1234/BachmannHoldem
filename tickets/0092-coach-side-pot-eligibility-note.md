...
- [x] When the hero is all-in for less than at least one other live contributor (i.e. the hero is
...
- [x] The eligibility logic reads from the engine (`pot.eligibleSeats` / the hero's `totalCommitted`
      vs others') — no re-implementation of the side-pot split in the coach layer.
- [x] The note fires only when a side pot the hero is excluded from actually exists; an even all-in
      (everyone eligible for one pot) produces no note. No false positives on ordinary single-pot
      all-ins.
- [x] Copy matches the coaching engine's existing voice/format and routes through the same
...
- [x] Tests in `packages/coach` cover: the note fires for a hero short all-in with a real side pot,
...
**As built.** Trigger is `shortAllInNote` in `packages/coach/src/verdict.ts`: fires only when the
action puts the hero all-in AND **≥ 2** live opponents are already in for more than the hero's final
total (one above = a returned uncalled bet, not a side pot — the false-positive guard). It reads
`DecisionContext` commitments (`pot` − opponents' `totalCommitted`, capped per seat), not the engine's
pot split. The note rides on a new `DecisionVerdict.shortAllIn` field and is rendered by
`explainDecision` (`@holdem/format`), the existing per-decision "why" path — so all clients get it.

**Review catch.** An open-shove into an _unbet_ street (`toCall === 0`, `heroBet`) is still a short
all-in when opponents went all-in on a prior street; the note was first dropped on that branch and is
now appended there too (regression-tested in `coachValues.test.ts`).

**Glossary.** "Main pot" / "Side pot" added as a browsable "Pots" section in `GlossaryOverlay`.
`GLOSSARY_TERMS` is keyed exhaustively by the coach's `GradeTermId`, so the terms live in the overlay's
section list, not that record. Inline-linking them inside the plain-text decision why-line is out of
scope (that segmented affordance is for grade explanations); the definition is browsable, not tappable.
